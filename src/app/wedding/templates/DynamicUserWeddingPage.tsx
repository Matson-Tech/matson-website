...
function DynamicUserWeddingPage() {
  ...
  useEffect(() => {
    ...
    const accessToken = params.get('access_token') ?? session?.access_token ?? '';
    const refreshToken = params.get('refresh_token') ?? session?.refresh_token ?? '';
  ...
  const handleFieldChange = useCallback(async (section: keyof WeddingData, field: string, value: string) => {
  ...
  const handlePendingChange = useCallback((section: keyof WeddingData, field: string, value: string) => {
    setPendingChanges(prev => {
      const sectionData = { ...(prev[section] as any ?? weddingData?.[section] as any ?? {}) };
